...
def prepare_for_w2v(filename_from, filename_to, lang):
    ...
    with open(filename_to, 'w', encoding='utf-8') as f:  # запись в файл векторов слов
        for sentence in nltk.sent_tokenize(raw_text, lang):  # преобразуем в векторы
            print(preprocess_text(sentence.lower()), file=f)  # перенаправляем поток вывода из файла
...
# необходимо построить весь словарь
prepare_for_w2v(r"base.txt", r"train.txt", r"russian")
model_ak = train_word2vec(r"train.txt")
...
prepare_for_w2v(r"base.txt", r"train.txt", r"russian")
model_wp = train_word2vec(r"train.txt")
